...
export default function CoachPerformance() {
  ...
  const handleSubmitAssessment = async () => {
    ...
    try {
      ...
      if (selectedHistoryPlayer?._id === activePlayer._id) {
  ...
  const handleOpenHistory = async (player) => {
    ...
    try {
      const res = await api.get(`/performance/player/${player._id}`);
      ...
      infoAlert(
        "No performance data found. Please assess the player first to view their history.",
      );
  ...
  const currentSkillsData = Object.entries(drillMetrics).map(
    ([name, value]) => ({
      ...
      <Card className="border-0 shadow-sm rounded-4 mb-4">
        <Card.Body className="p-0">
          <div className="p-4 border-bottom d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 bg-light rounded-top-4">
            <h6
              className="fw-bold mb-0 text-uppercase text-muted small d-flex align-items-center"
          ...
          <div className="table-responsive">
            <Table hover className="mb-0 align-middle custom-table">
              <thead className="bg-white">
                <tr>
                  <th className="text-muted small fw-semibold text-uppercase px-4 py-3 border-0">
                    Athlete Name
                  </th>
                  <th className="text-muted small fw-semibold text-uppercase py-3 border-0">
                    Position
                  </th>
                  <th className="text-muted small fw-semibold text-uppercase text-end px-4 py-3 border-0">
                    Actions
                  </th>
                </tr>
              </thead>
                ...
                {loadingPlayers ? (
                  ...
                  <tr>
                    <td colSpan="3" className="text-center py-5 text-muted">
                      ...
                      <p className="mb-0 fw-medium">
                        No players found in this squad category.
                      </p>
                    </td>
                  </tr>
                ) : (
                  players.map((p) => (
                    <tr
                      key={p._id}
                      className={
                        selectedHistoryPlayer?._id === p._id
                    ...
                    >
                      ...
                      <td className="py-3">
                        <Badge
                          bg="light"
                          text="dark"
                          className="border fw-medium px-3 py-2 rounded-pill"
                        >
        ...
        <Card className="border-0 shadow-lg rounded-4 mb-5 overflow-hidden slide-up-animation">
          <div className="border-bottom bg-white px-4 py-3 d-flex justify-content-between align-items-center">
            <div className="d-flex align-items-center gap-3">
              ...
              >
                {selectedHistoryPlayer.firstName.charAt(0)}
                {selectedHistoryPlayer.lastName.charAt(0)}
              </div>
              <div>
                <h5 className="fw-bolder mb-0 text-dark">
                  {selectedHistoryPlayer.firstName}{" "}
                  {selectedHistoryPlayer.lastName}
                </h5>
                <span
            ...
            <button
              className="btn btn-light border shadow-sm rounded-circle p-2 d-flex align-items-center justify-content-center text-muted"
                    ...
                    <div className="card border-0 shadow-sm rounded-4 h-100 bg-white">
                      <div className="card-body p-4 d-flex align-items-center gap-3">
                        ...
                        <div>
                          ...
                          <h3 className="fw-black mb-0 text-dark lh-1">
                            {historyData.metrics?.averageRating?.toFixed(1) ||
                              "0.0"}{" "}
                            <span className="fs-6 text-muted fw-medium">
                              / 10
                            </span>
                          </h3>
                        </div>
                    ...
                    <div className="card border-0 shadow-sm rounded-4 h-100 bg-white">
                      <div className="card-body p-4 d-flex align-items-center gap-3">
                        ...
                        <div>
                          ...
                          <h3 className="fw-black mb-0 text-dark lh-1">
                            {historyData.metrics?.score?.toFixed(0) || "0"}{" "}
                            <span className="fs-6 text-muted fw-medium">
                              pts
                            </span>
                          </h3>
      ...
      >
        ...
        <Modal.Body className="px-4 py-4">
          <div className="d-flex flex-column gap-3">
            {(SPORT_DRILLS[sport] || []).map((drill) => {
              ...
              return (
                <div
                  key={drill}
                  className="p-3 bg-light rounded-4 border border-light"
                >
                  <div className="d-flex justify-content-between align-items-center mb-2">
                    <Form.Label className="mb-0 fw-bold text-dark small text-capitalize">
                      {label}
                    </Form.Label>
                    <span
          ...
          <div className="mt-4 p-3 bg-primary bg-opacity-10 rounded-4 d-flex justify-content-between align-items-center border border-primary border-opacity-25">
            <span
              className="fw-bold text-primary small text-uppercase"
              style={{ letterSpacing: "0.5px" }}
            >
              Session Average
            </span>
            <span className="fs-3 fw-black text-primary lh-1">
              {currentSessionAvg}
            </span>
          </div>
        </Modal.Body>
        ...
        <Modal.Footer className="border-0 px-4 pb-4 pt-0">
          ...
          <Button
            variant="primary"
            className="fw-bold rounded-pill px-4 d-flex align-items-center gap-2 shadow-sm"
