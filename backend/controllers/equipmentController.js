...
exports.createEquipment = async (req, res) => {
  try {
    const { name, description = "", quantityTotal = 0, location } = req.body;
    if (!name || quantityTotal == null) {
      return res.status(400).json({ message: "Missing name or quantityTotal" });
    }
...
exports.requestEquipment = async (req, res) => {
  try {
    ...
    const { equipmentId, quantityRequested, scheduleId, reason = "" } = req.body;

    if (!equipmentId || !quantityRequested) {
      return res.status(400).json({ message: "Missing equipmentId or quantityRequested" });
    }
    ...
    return res.status(201).json({ message: "Equipment request submitted", request });
  } catch (err) {
...
exports.reportDamage = async (req, res) => {
  try {
    ...
    const { equipmentId, quantityDamaged, description = "", severity = "low", scheduleId } = req.body;

    if (!equipmentId || !quantityDamaged) {
      return res.status(400).json({ message: "Missing equipmentId or quantityDamaged" });
    }
    ...
    await Equipment.findByIdAndUpdate(equipmentId, {
      $inc: { quantityDamaged: quantityDamaged, quantityAvailable: -Math.abs(quantityDamaged) }
    });
...
exports.listRequests = async (req, res) => {
  try {
    const requests = await EquipmentRequest.find().populate("equipmentId coachId").sort({ createdAt: -1 }).lean();
    res.json({ requests });
  } catch (err) {
...
exports.processRequest = async (req, res) => {
  try {
    ...
    if (action === "approve") {
      ...
      const qtyToApprove = Math.min(reqDoc.quantityRequested, approvedQuantity || reqDoc.quantityRequested);
      ...
      if (qtyToApprove <= 0) {
        return res.status(400).json({ message: "No quantity available to approve" });
      }
      ...
      return res.json({ message: "Request partially approved", request: reqDoc });
    } else {
      // reject
